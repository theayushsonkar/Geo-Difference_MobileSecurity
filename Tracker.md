...
══════════════════════════════════════════════════════════════
[1] scrapper.py
      └─ Queries Google Play by country + category
      └─ Output: output/top_apps_full.csv
         │
[2] pipeline/prepare_package_list.py
      └─ Filters corpus to target market + ranks by popularity
      └─ Output: data/package_lists/<country>_packages.txt
         │
[3] pipeline/download_apks.py (apkeep)
      └─ Downloads .apk / split-APK bundles
      └─ Output: apks/<package_name>/
         │
[4] pipeline/normalize_packages.py
      └─ Extracts XAPK → pure .apk; verifies file integrity
      └─ Output: normalized/<package_name>/
         │
[5] pipeline/decode_apks.py (Apktool 3.0.1)
      └─ Disassembles DEX → Smali; decodes AndroidManifest.xml
      └─ Output: decoded/<package_name>_decoded/
         │
[6] pipeline/build_sample_index.py
      └─ SHA-256 hash, country code, store metadata registered
      └─ Output: sample_index.csv  ◄─── JOIN KEY for all downstream

Phase B — Static Analysis
══════════════════════════════════════════════════════════════
         ┌─────────────────────────────────────────┐
         │                                         │
[7] scan_manifest.py                        [9] sdk_detection/
   manifest_scanner/                             LibScanRunner
      ├─ PrivacyMatcher (Aho-Corasick)              └─ Chunked JVM subprocess
      ├─ SecretMatcher (TruffleHog regexes)     FallbackDetector
      └─ GeoMatcher (FlowDroid sinks)               └─ Longest-prefix Smali match
         │                                      Canonicalizer → TrackerEnricher
         │                                      MetadataLoader (vendor/country)
[8] cve/main.py                                      │
      └─ NVD ZIP → CVERecord index                   │
      └─ SDK version → CPE constraint match          │
         │                                           │
         └──────────────────┬──────────────────────┘
                            │
              manifest_sdks.csv, permissions.csv,
              components.csv, sdk_cve_matches.csv

Phase C — Dynamic Analysis
══════════════════════════════════════════════════════════════
[10] collect_pcap.py
      └─ ADB install → PCAPdroid start → Monkey (500 events, 60s)
      └─ PCAPdroid stop → ADB pull → ADB uninstall
      └─ Output: data/pcap/<sample_id>.pcap
         │
[11] run_pcap_analysis.py
      pcap/pcap_parser.py       → list[RawEvent]
      pcap/connection_builder.py → list[ConnectionRecord]  (6-tuple)
      pcap/geoip.py              → country_code, ASN per IP
      pcap/app_summary.py        → per-app aggregate metrics
      └─ Output: pcap_connections.csv, pcap_dns.csv,
                 pcap_domain_geo.csv, pcap_app_summary.csv

Phase D — Analysis & Reporting (Pending)
══════════════════════════════════════════════════════════════
[12] Dataset Integration   →  Unified CSV joined on sample_id
[13] Statistical Analysis  →  SDK/tracker/CVE/network comparison
[14] Visualization         →  Charts, heatmaps, geo plots
[15] Thesis / Paper        →  Methodology, results, discussion
```
...
| Smali scan directories | `smali/`, `smali_classes2/`, `smali_classes3/` … | Multi-dex APK support |
| Tracker matching algorithm | Deterministic longest-suffix domain match | Zero false positives; no heuristics |
| LibScan priority | LibScan > FallbackDetector | Bytecode signatures more accurate than string prefix |
| Private IP exclusions | `10.0/8`, `172.16/12`, `192.168/16`, `127.0/8`, `169.254/16` | Exclude local routing hops from GeoIP country analysis |
| Sample identity key | `{package_name}_{country_code}` = `sample_id` | Clean, human-readable join key across all 14 output datasets |
| SDK canonicalization | Custom `sdk_metadata.csv` vendor alias map | Prevents double-counting Google sub-SDKs (Maps, Firebase, etc.) |
| TruffleHog patterns | 700+ regex rules from open-source ruleset | Covers AWS, GCP, Azure, Stripe, Twilio, GitHub tokens |
| NVD coverage | 2002–2026 (25 annual ZIP feeds) | Full historical CVE catalog for maximum recall |
| GeoIP cache | Single in-process LRU dict in `GeoMapper` | No secondary caches; eliminates dual-source-of-truth bugs |
...
| Network Traffic Analysis — cross-country aggregation | Individual PCAP CSVs generated; final JOIN + country-level aggregation pending |
